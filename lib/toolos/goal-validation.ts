import type { AbsenceReason, GoalPlan, GoalSatisfactionResult, Decision } from "@/lib/toolos/spec";
...
export function evaluateGoalSatisfaction(params: {
  prompt: string;
  goalPlan?: GoalPlan;
...
export function buildAbsenceExplanation(result: GoalSatisfactionResult): string {
  switch (result.absence_reason) {
    ...
    case "integration_permission_missing":
      return "Required integrations are missing permissions to complete this request.";

    ...
    private registerCapabilities() {
        ...
        this.capabilities["github_issues_search"] = {
            ...
            execute: async (params, context, trace) => {
                ...
                // Scope search to authenticated user's repos only
                // Add user: filter if not already present in query
                const queryStr = params.q || '';
                let scopedQuery = queryStr;
                if (owner && queryStr && !queryStr.includes('user:') && !queryStr.includes('org:') && !queryStr.includes('repo:')) {
                    scopedQuery = `user:${owner} ${queryStr}`;
                }

                const query = new URLSearchParams();
                query.append("q", scopedQuery);
